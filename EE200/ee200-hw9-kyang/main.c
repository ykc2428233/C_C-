...
{
  FILE* empty = fopen("empty.mon", "r");
  Element* em = load_tree(empty);
  fclose(empty);
  free_tree(em);

  FILE* corrup = fopen("corrupt_test.mon", "r");
  Element* co = load_tree(corrup);
  fclose(corrup);
  free_tree(co);

  FILE* n_mon = fopen("n_test.mon", "r");
  Element* n = load_tree(n_mon);
  fclose(n_mon);
  free_tree(n);

  FILE* n_mon_r = fopen("n_test_r.mon", "r");
  Element* nr = load_tree(n_mon_r);
  fclose(n_mon_r);
  free_tree(nr);

  FILE* n_mon_l = fopen("n_test_l.mon", "r");
  Element* nl = load_tree(n_mon_l);
  fclose(n_mon_l);
  free_tree(nl);
